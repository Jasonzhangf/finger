...
1. **发现不等于结论**：当发现问题现象时，不要立刻决定修复行为。
2. **真源追踪优先**：必须追溯问题根源，进行多层级真源判定：
   - 第一层：现象出现在哪里？（文件/模块/接口）
   - 第二层：现象的真源在哪里？（配置/数据/逻辑）
   - 第三层：真源是否唯一？（是否存在其他影响点）
3. **唯一真源确认**：只有确认是唯一的问题真源，才能进行修复决策。
4. **全局唯一修改点**：修复和设计修改必须确定是全局唯一真源、唯一正确的位置，避免多源污染。
5. **谋定而后动**：在修改前必须：
   - 完成真源追踪
   - 确认唯一修改点
   - 验证修改范围不溢出
