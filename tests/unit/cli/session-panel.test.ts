...
describe('session-panel helpers', () => {
  it('uses explicit ws url when provided', () => {
    expect(deriveWsUrl('http://localhost:9999', 'ws://localhost:18888')).toBe('ws://localhost:18888');
  });

  it('derives ws url from daemon url', () => {
    expect(deriveWsUrl('http://127.0.0.1:9999')).toBe('ws://127.0.0.1:9998');
    expect(deriveWsUrl('https://finger.local')).toBe('wss://finger.local:9998');
  });

  it('falls back to default ws url when daemon url is invalid', () => {
    expect(deriveWsUrl('not-a-url')).toBe('ws://localhost:9998');
  });
