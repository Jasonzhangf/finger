...
export class SessionManager {
 ...
 ensureSession(sessionId: string, projectPath: string, name?: string): Session {
  ...
  async compressContext(sessionId: string, options?: { force?: boolean }): Promise<string> {
    ...
    try {
      // 使用 Rust finger-mem 进行 task-level digest 压缩
      const cliPath = path.join(process.env.HOME || "/Users/fanzhang", "code", "finger-mem", "target", "release", "finger-mem");

      // Step 1: Compact ledger → digest
      ...
      // Step 2: Rebuild context with budget
      const rebuildStdout = await new Promise<string>((resolve, reject) => {
        execFile(cliPath, [
          "rebuild",
          "--session", sessionId,
          "--new-topic-tags", "context compact",
          "--budget", "20000",
        ], { timeout: 30000 }, (err, stdout, stderr) => {
          if (err) reject(err);
          else resolve(stdout);
      ...
      // Parse output
      const tokensMatch = rebuildStdout.match(/tokens.*?(\d+)/i);
      const tasksMatch = rebuildStdout.match(/tasks?.*?(\d+)/i);
      ...
      log.info("Compressed session with Rust finger-mem", {
        sessionId,
        tokensUsed,
        taskCount,
      });
      ...
      log.error(`Rust finger-mem CLI failed for session ${sessionId}:`, error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
/**
 ...
 ensureSession(sessionId: string, projectPath: string, name?: string): Session {
  ...
  async compressContext(sessionId: string, options?: { force?: boolean }): Promise<string> {
    ...
    try {
      ...
      log.error(`Rust ledger-cli failed for session ${sessionId}:`, error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
