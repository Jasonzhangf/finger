...
- SHELL_EXEC: 执行 shell 命令，参数 { command: "命令" }
- FETCH_URL: 获取网页内容，参数 { url: "网页地址", saveTo: "保存路径(可选)" }
...
只输出 JSON，不要其他文字：
{"thought": "分析", "action": "WRITE_FILE|SHELL_EXEC|FETCH_URL|COMPLETE|FAIL", "params": {"path": "xxx", "content": "xxx"}}

示例：
{"thought": "需要创建文件", "action": "WRITE_FILE", "params": {"path": "/tmp/test.txt", "content": "Hello"}}
{"thought": "搜索信息", "action": "SHELL_EXEC", "params": {"command": "curl -s 'https://api.example.com/search?q=deepseek'"}}
{"thought": "获取网页", "action": "FETCH_URL", "params": {"url": "https://example.com", "saveTo": "/tmp/page.html"}}
{"thought": "任务完成", "action": "COMPLETE", "params": {"output": "文件已创建"}}`;
 ...
 const agent = new Agent({
  ...
  function isConnectionError(error?: string): boolean {
    ...
    return normalized.includes('not connected') || normalized.includes('call connect') || normalized.includes('connect() first');
  }
  ...
  async function reactLoop(
    ...
    if (!initialized) {
      ...
      if (!initPromise) {
        initPromise = agent.initialize().then(status => {
...
请立即输出 JSON 格式的决策（只输出 JSON，不要其他文字）：`;
      ...
      if (!decision.success && isConnectionError(decision.error)) {
        console.warn(`[ExecutorLoop ${config.id}] Connection error detected, trying to reconnect...`);
        logger.log({
          timestamp: new Date().toISOString(),
        ...
        try {
          ...
          console.log(`[ExecutorLoop ${config.id}] Reconnected, connected: ${status.connected}`);
          initialized = true;
          ...
          const reconnectError = reconnectErr instanceof Error ? reconnectErr.message : String(reconnectErr);
          logger.log({
            timestamp: new Date().toISOString(),
            iteration: state.iteration,
          ...
          return { success: false, error: `Reconnect failed: ${reconnectError}` };
        }
      }
