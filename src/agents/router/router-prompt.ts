...
你必须严格返回以下 JSON 格式（不要包含 markdown 代码块标记，不要有任何其他文字）：
...
export function parseRouterDecision(response: string): {
  ...
  try {
    ...
    // 方式 1: 尝试提取 code block 中的 JSON (优先)
 ...
 * 构建决策结果对象
 */
function buildDecisionResult(decision: any): {
  intent: { type: string; confidence: number; reasoning: string };
  target: { agentId: string; agentName: string; matchedCapabilities: string[]; reasoning: string };
  metadata: {
    inputSummary: string;
  ...
  // 验证必要字段
  if (!decision.intent && !decision.target && !decision.classification) {
    if (decision.type) {
      decision.intent = { type: decision.type, confidence: 0.8, reasoning: 'inferred' };
    }
    if (decision.agentId || decision.targetModule) {
      decision.target = {
        agentId: decision.agentId || decision.targetModule,
        agentName: decision.agentName || decision.targetModule,
        matchedCapabilities: [],
        reasoning: 'inferred'
      };
    }
  ...
  if (!decision.intent && !decision.classification) {
    return null;
  }
  if (!decision.target && !decision.targetModule && !decision.agentId) {
    return null;
  }

  const intent = decision.intent || decision.classification || {};
  const target = decision.target || {
    agentId: decision.targetModule || decision.agentId,
    agentName: decision.agentName || decision.targetModule || decision.agentId,
    matchedCapabilities: decision.matchedCapabilities || decision.requiredCapabilities || [],
  ...
  return {
    intent: {
      ...
      confidence: parseFloat(intent.confidence) || 0.5,
      reasoning: intent.reasoning || '',
    },
    target: {
      agentId: target.agentId || 'task-orchestrator',
      agentName: target.agentName || target.agentId || 'task-orchestrator',
      matchedCapabilities: Array.isArray(target.matchedCapabilities) ? target.matchedCapabilities : [],
      reasoning: target.reasoning || '',
    },
    metadata: {
      inputSummary: decision.metadata?.inputSummary || decision.inputSummary || '',
      keyFeatures: Array.isArray(decision.metadata?.keyFeatures) ? decision.metadata.keyFeatures : (Array.isArray(decision.keyFeatures) ? decision.keyFeatures : []),
      alternativeTargets: Array.isArray(decision.metadata?.alternativeTargets) ? decision.metadata.alternativeTargets : (Array.isArray(decision.alternativeTargets) ? decision.alternativeTargets : []),
      requiresHumanReview: decision.metadata?.requiresHumanReview || decision.requiresHumanReview || false,
    },
  };
}
