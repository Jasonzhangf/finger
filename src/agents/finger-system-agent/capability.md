...
- `session.switch` - **已禁用**（系统策略禁止运行期切会话）
...
#### 权限规则
- **所有 agent（包含 System Agent）**：运行期禁止 session 切换
- session 仅允许在 runtime 初始化阶段绑定

#### 用户使用方式
用户可请求列会话；若要切换，会被提示当前策略已禁用运行期切会话。
...
- `<##@system:stopall##>` - 强制停止所有 Agent 当前推理（中断 active turns）
- `<##@agent##>` - 切换到业务 orchestrator
- `<##@project:list##>` - 列出所有项目
- `<##@project:switch@/path##>` - 切换项目
- `<##display:"show"##>` - 查看当前渠道 UI 刷新配置
- `<##display:"ctx:on|off|simple|verbose"##>` - 上下文显示复杂度
- `<##display:"mode:progress|command|both"##>` - 更新模式（进度/命令/两者）
- `<##display:"reasoning:on|off"##>` - 推理流显示
- `<##display:"body:on|off"##>` - 正文增量显示
- `<##display:"status:on|off"##>` - 状态消息显示
- `<##display:"toolcall:on|off"##>` - 工具调用显示
- `<##display:"step:on|off"##>` - step 更新显示
- `<##display:"stepbatch:1-50"##>` - step 批次大小
- `<##display:"progress:on|off"##>` - 周期进度更新显示
- `<##display:"heartbeat:on|off"##>` - 心跳更新显示
